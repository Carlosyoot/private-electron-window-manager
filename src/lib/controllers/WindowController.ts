  ...
  public open<T extends object>(WindowItem: ClassRef<T>, options?: { parent?: WindowRef<any>, modal?: boolean }): T {
    ...
    config.ipcEvents.forEach(({ channel, callback }) => {
        const handler = (event: Electron.IpcMainEvent, ...args: any[]) => {
            // Filtro de Segurança: Só aceita se vier desta janela
            if (event.sender.id === windowId) {
                // CORREÇÃO DOS ARGUMENTOS:
                // args[0] é o dado enviado pelo front (payload)
                // Se args estiver vazio, passamos undefined
                const payload = args.length > 0 ? args[0] : undefined;
                
                // Chamamos: (Evento, Janela, Dados)
                callback(event, nativeWin, payload);
            }
        };
    ...
    config.ipcHandlers.forEach(({ channel, callback }) => {
        const handler = (event: Electron.IpcMainInvokeEvent, ...args: any[]) => {
             if (event.sender.id === windowId) {
                const payload = args.length > 0 ? args[0] : undefined;
                return callback(event, nativeWin, payload);
            }
        };
    ...
    // 7. Registro e Limpeza
    // NOVO: Passamos o windowId explicitamente para o Metadata
    ...
    nativeWin.on('closed', () => {
        ...
        // Agora o remove funciona pois usa o windowId salvo, não tenta ler a janela morta
